...
def compute_lpips(lpips_network, ref_img: np.ndarray, deg_img: np.ndarray) -> float:
    ...
    ref_img_rgb = (
        grey_to_rgb(ref_img)
        if ref_img.ndim == 2
        else torch.tensor(ref_img[None, :, :, :], dtype=torch.float32)
    )
    deg_img_rgb = (
        grey_to_rgb(deg_img)
        if deg_img.ndim == 2
        else torch.tensor(deg_img[None, :, :, :], dtype=torch.float32)
    )

    return float(
        lpips_network.forward(ref_img_rgb, deg_img_rgb).detach().numpy()[0, 0, 0, 0]
    )
...
def compute_information_weighted_ssim(
    ref_img: np.ndarray, deg_img: np.ndarray
) -> float:
    ref_tensor = np_to_tensor(ref_img)
    deg_tensor = np_to_tensor(deg_img)
...
def np_to_tensor(image: np.ndarray) -> torch.Tensor:
    ...
    if len(image.shape) == 2:  # Grayscale image
        image_tensor = torch.tensor(
            image[None, None, :, :], dtype=torch.float32
        )  # Add channel and batch dimensions
    elif len(image.shape) == 3:  # RGB image
        image_tensor = torch.tensor(
            image[None, :, :, :], dtype=torch.float32
...
def grey_to_rgb(image_grey: np.ndarray) -> torch.Tensor:
    ...
    if len(image_grey.shape) == 2:  # Check if it's a grayscale image
        ...
        len(image_grey.shape) == 3 and image_grey.shape[0] == 3
    ):  # Already an RGB image
        return torch.tensor(
            image_grey[None, :, :, :], device="cpu", dtype=torch.float32
        )
    else:
...
def process_images(
    synthetic_dir: str, real_dir: str, metric_func: Callable, metric_name: str
) -> Dict[str, float]:
    """Compute a specified metric for synthetic images against all real images and aggregate results."""
    metric_values = {}

    real_images = [
        f for f in os.listdir(real_dir) if os.path.isfile(os.path.join(real_dir, f))
    ]
    ...
    synthetic_files = [
        ...
        if os.path.isfile(os.path.join(synthetic_dir, f))
    ]
    for synthetic_file in tqdm(
        synthetic_files, desc=f"Computing {metric_name}", unit="image"
    ):
        ...
        if os.path.isfile(synthetic_path):
            ...
            for real_image_file in tqdm(
                real_images, desc="Real Images", unit="file", leave=False
            ):
...
def compute_non_ref_metrics(
    synthetic_dir: str, pyiqa_metrics: List[str]
    ...
    for metric_name in pyiqa_metrics:
        ...
        for synthetic_file in tqdm(
            ...
            if os.path.isfile(synthetic_path):
                ...
                synthetic_img = synthetic_img.resize(
                    (512, 512), Image.Resampling.LANCZOS
                )
                # Compute the metric
                output = metric_model(synthetic_img)
                metric_results.append(
                    {"image": synthetic_file, metric_name: output.item()}
                )
...
def main_single_metric_eval(
    synthetic_images_dir: str, real_images_dir: str, output_dir: str, model_to_seek: str
) -> None:
    ...
    for metric_name, metric_func in metrics.items():
        metric_values = process_images(
            synthetic_images_dir, real_images_dir, metric_func, metric_name
        )

        # Convert metric results to DataFrame format and append to the list
        metric_df = pd.DataFrame.from_dict(
            metric_values, orient="index", columns=[metric_name]
        )
    ...
    if os.path.exists(os.path.join(output_dir, "single_metric_eval_baseline.csv")):

        non_ref_baseline = compute_non_ref_metrics(real_images_dir, pyiqa_metrics)

        all_metric_baseline = []

        for metric_name, metric_data in non_ref_baseline.items():
            # Prepare DataFrame from the metric data
            metric_df = pd.DataFrame.from_records(metric_data).set_index("image")

            metric_df.index.name = "Image"
            all_metric_baseline.append(metric_df)

    # Concatenate all metrics DataFrames into a single DataFrame
    result_df = pd.concat(all_metrics, axis=1)
    base_df = pd.concat(all_metric_baseline, axis=1)

    base_df.to_csv(os.path.join(output_dir, "single_metric_eval_baseline.csv"))
    # Save the DataFrame to a CSV file
    result_df.to_csv(
        os.path.join(output_dir, f"{model_to_seek}_single_metric_eval.csv")
    )


def compute_ground_truth_correlations(
    output_dir: str,
    mean_realism_z_scored: pd.DataFrame,
    do_z_score: bool,
    model_to_seek: str,
) -> None:
    # Define the paths for the results files
    single_metric_eval_path = os.path.join(
        output_dir, f"{model_to_seek}_single_metric_eval.csv"
    )
    single_metric_eval_baseline_path = os.path.join(
        output_dir, "single_metric_eval_baseline.csv"
    )

    if not os.path.exists(single_metric_eval_path) or not os.path.exists(
        single_metric_eval_baseline_path
    ):
        print(
            f"Output files not found in {output_dir}. Ensure that 'main_single_metric_eval' has been run."
        )
        return

    # Load the metric evaluation results
    single_metric_df = pd.read_csv(single_metric_eval_path, index_col="Image")
    single_metric_baseline_df = pd.read_csv(
        single_metric_eval_baseline_path, index_col="Image"
    )
    ...
    for metric in single_metric_df.columns:
        ...
        pearson_corr = combined_df[metric].corr(
            combined_df[realism_column], method="pearson"
        )
        kendall_corr = combined_df[metric].corr(
            combined_df[realism_column], method="kendall"
        )
...
def plot_metric_comparison(
    ...
    single_metric_baseline_df: pd.DataFrame,
    output_dir: str,
    model_to_seek: str,
) -> None:
    # Ensure that the dataframes have the same metrics
    common_metrics = list(
        set(single_metric_df.columns) & set(single_metric_baseline_df.columns)
    )
    ...
    for i, metric in enumerate(common_metrics):
        ...
        sns.kdeplot(
            single_metric_baseline_df[metric], ax=axs[i], label="Real", color="blue"
        )
    ...
    plt.savefig(
        os.path.join(
            output_dir, f"{model_to_seek}_metric_comparison_real_vs_synthetic.png"
        )
    )
