...
from tensorflow.keras.applications.inception_resnet_v2 import (
    preprocess_input as inception_resnet_preprocess,
)
    ...
    def __init__(
        ...
        if self.backbone_type == "torch":
            ...
            if isinstance(self.backbone.config, CLIPConfig):
                ...
                raise ValueError(
                    f"Unsupported model configuration: {type(self.backbone.config)}"
                )
            ...
            self.backbone_type == "keras"
        ):  ##this is deprecated and trianign with keras models is not possible all teh trianing logic is in torc issue: network graphh
            # Keras model handling remains the same
            base_model = Model(
                inputs=self.backbone.input, outputs=self.backbone.layers[-2].output
            )
    ...
    def forward(self, input1=None, input2=None, resnet_only=False):
        if resnet_only:
            if self.backbone_type == "torch":
                ...
                raise ValueError(
                    f"resnet_only not supported for backbone type: {self.backbone_type}"
                )
        else:
    ...
    def forward(self, x):
        ...
        if hasattr(self.backbone, "config") and isinstance(
            ...
            features = self._get_features(
                self.backbone.vision_model(x.to(self.device))
            ).to(self.device)

        else:
            features = self._get_features(self.backbone(x.to(self.device))).to(
                self.device
            )
...
def load_and_remap_state_dict(model, filename, repo_id="molinamarc/syntheva"):
    try:
        ...
        try:
            ...
            print(
                f"Couldn't load {filename} with strict=True. Trying with strict=False. Error: {str(e)}"
            )
...
def initialize_model(network_name):
    ...
    if network_name.lower() == "resnet50":
        ...
        backbone = AutoModel.from_pretrained(
            "microsoft/rad-dino", output_hidden_states=True
        )
        ...
        backbone = models.inception_v3(pretrained=False)
        backbone = load_and_remap_state_dict(
            backbone, "RadImageNet-InceptionV3_notop.pth"
        )
        backbone_type = "torch"
        ...
        backbone = models.densenet121(pretrained=False)
        backbone = load_and_remap_state_dict(
            backbone, "RadImageNet-DenseNet121_notop.pth"
        )
        backbone_type = "torch"
...
class ContrastiveDataset(torch.utils.data.Dataset):
    def __init__(
        self, file_paths, target_resolution, transforms, num_channels=3, inference=False
    ):
...
def find_executable_batch_size(
    function: callable = None, starting_batch_size: int = 32
):
    if function is None:
...
def setup_training(root_dir, network_name, **kwargs):
    ...
    if rank == 0:
        ...
        train_paths = convert_to_npy(
            train_paths,
            os.path.join(config["output_preprocessing"], "train"),
            target_res,
        )
        val_paths = convert_to_npy(
            val_paths, os.path.join(config["output_preprocessing"], "val"), target_res
        )
    ...
    dist.broadcast_object_list(
        data_to_broadcast, src=0, device=torch.cuda.current_device()
    )
    ...
    try:
        ...
        model = ContrastiveNetwork(
            ...
            attentive_probing=config["attentive_probing"],
            device=device,
        )
        ...
        [
            ...
            tio.RandomGamma(
                log_gamma=(
                    -0.1,
                    0.1,
                ),  # Reduced range for less intense gamma correction
                p=0.4,
            ),
    ...
    TRAIN_TRANSFORMS = tio.Compose([MANDATORY_TRANSFORMS, AUGMENTATION_TRANSFORMS])
    ...
    train_loader = MultiEpochsDataLoader(train_dataset, **loader_args)
    ...
    val_loader = MultiEpochsDataLoader(val_dataset, **loader_args)
    ...
    if config["loss_type"].lower() == "ntxent":
        ...
        raise ValueError(
            f"Unsupported loss type: {config['loss_type']}. Choose 'ntxent' or 'triplet'."
        )
    ...
    try:
        ...
        for epoch in range(config["n_epochs"]):
            ...
            if epoch == config["unfreeze_epoch"]:
                ...
                if "dino" in network_name or "clip" in network_name:
                    optimizer = torch.optim.AdamW(
                        model.parameters(),
                        lr=config["base_lr"] / config["downscaling_after_freezing"],
                    )
            ...
            val_loss, pos_sim, euc_pos_dist, euc_neg_dist = validate(
                model, val_loader, device, LossTr, config, miner
            )
...
def train_epoch(
    ...
    for train_step, batch in progress_bar:
        ...
        if Pos11.dim() != 4:
            raise ValueError(
                f"Expected 4D input, got {Pos11.dim()}D with shape {Pos11.shape}"
            )
        ...
        if config["loss_type"].lower() == "infonce":
            ...
            negative_embeddings = embeddings[negatives].view(
                len(anchors), -1, embeddings.shape[1]
            )
...
def validate(model, val_loader, device, LossTr, config, miner=None):
    ...
    for batch in val_loader:
        ...
        euc_neg_dist.append(
            euclidean_distance_neg[np.triu_indices_from(euclidean_distance_neg, k=1)]
        )
...
def load_best_model_for_inference(
    network_name, config, checkpoints_dir="./checkpoints", batch_size=32, num_workers=4
):
    ...
    if config["contrastive"]:
        ...
        network_checkpoints = glob.glob(
            os.path.join(checkpoints_dir, f"*{config['loss_type']}_{network_name}")
        )
        ...
        json_pattern = os.path.join(
            latest_checkpoint_dir, f"*{network_name}_image_paths.json"
        )
    ...
    def create_strong_augmentation(self):
        return tio.Compose(
            [
                ...
                Custom2DRotation(degrees=20, p=0.7),
                tio.RandomAffine(
                    degrees=0, scales=(0.8, 1.2), default_pad_value="minimum", p=0.5
                ),
                tio.RandomFlip(
                    axes=(2), flip_probability=0.5
                ),  # Changed from (0, 1) to (1, 2)
                tio.RandomBiasField(coefficients=0.3, order=3, p=0.4),
                tio.RandomGamma(log_gamma=(-0.1, 0.1), p=0.4),
...
def create_dataloaders(train_paths, val_paths, synthetic_paths, config):
    ...
    train_paths = convert_to_npy(
        train_paths, os.path.join(config["output_preprocessing"], "train"), target_res
    )
    val_paths = convert_to_npy(
        val_paths, os.path.join(config["output_preprocessing"], "val"), target_res
    )
    synthetic_paths = convert_to_npy(
        synthetic_paths,
        os.path.join(config["output_preprocessing"], "synth"),
        target_res,
    )
...
def inference_and_save_embeddings(
    model,
    device,
    train_standard_loader,
    val_standard_loader,
    synth_loader,
    network_name,
    output_dir,
    ...
    try:
        ...
        (
            train_standard_embeddings,
            train_standard_ids,
            train_adversarial_embeddings,
            train_adversarial_ids,
        ) = process_dataloader(
            train_standard_loader, "train_standard", model, device, adversarial=True
        )
        print()
        print("Processing val")
        (
            val_standard_embeddings,
            val_standard_ids,
            val_adversarial_embeddings,
            val_adversarial_ids,
        ) = process_dataloader(
            val_standard_loader, "val_standard", model, device, adversarial=True
        )
        print()
        print("Processing synthetic")
        synth_standard_embeddings, synth_standard_ids, _, _ = process_dataloader(
            synth_loader, "synth_standard", model, device, adversarial=False
        )

        output_file = os.path.join(
            output_dir,
            f"{network_name}_embeddings_aug_{config['augmentation_strength']}_{config['loss_type']}.h5",
        )
        with h5py.File(output_file, "w") as f:
            f.create_dataset(
                "train_standard/embeddings", data=train_standard_embeddings
            )
            f.create_dataset(
                "train_standard/image_ids",
                data=np.array(train_standard_ids, dtype=h5py.special_dtype(vlen=str)),
            )

            f.create_dataset(
                "train_adversarial/embeddings", data=train_adversarial_embeddings
            )
            f.create_dataset(
                "train_adversarial/image_ids",
                data=np.array(
                    train_adversarial_ids, dtype=h5py.special_dtype(vlen=str)
                ),
            )

            f.create_dataset("val_standard/embeddings", data=val_standard_embeddings)
            f.create_dataset(
                "val_standard/image_ids",
                data=np.array(val_standard_ids, dtype=h5py.special_dtype(vlen=str)),
            )

            f.create_dataset(
                "val_adversarial/embeddings", data=val_adversarial_embeddings
            )
            f.create_dataset(
                "val_adversarial/image_ids",
                data=np.array(val_adversarial_ids, dtype=h5py.special_dtype(vlen=str)),
            )

            f.create_dataset(
                "synth_standard/embeddings", data=synth_standard_embeddings
            )
            f.create_dataset(
                "synth_standard/image_ids",
                data=np.array(synth_standard_ids, dtype=h5py.special_dtype(vlen=str)),
            )
...
def compute_distances_and_plot(embeddings_file, output_dir, config, methods=None):
    if methods is None:
        methods = [
            "braycurtis",
            "canberra",
    ...
    print(
        f"Shapes: train_standard={train_standard_embeddings.shape}, "
    ...
    print(f"Computing distances/similarities for methods: {methods}")
    ...
    for method in methods:
        ...
        # For Jensen-Shannon distance, ensure the embeddings are non-negative and sum to 1
        if method == "jensenshannon":
            train_standard_embeddings_js = np.abs(train_standard_embeddings)
            train_standard_embeddings_js /= np.sum(
                train_standard_embeddings_js, axis=1, keepdims=True
            )
            val_standard_embeddings_js = np.abs(val_standard_embeddings)
            val_standard_embeddings_js /= np.sum(
                val_standard_embeddings_js, axis=1, keepdims=True
            )
            train_adversarial_embeddings_js = np.abs(train_adversarial_embeddings)
            train_adversarial_embeddings_js /= np.sum(
                train_adversarial_embeddings_js, axis=1, keepdims=True
            )
            val_adversarial_embeddings_js = np.abs(val_adversarial_embeddings)
            val_adversarial_embeddings_js /= np.sum(
                val_adversarial_embeddings_js, axis=1, keepdims=True
            )
            synth_standard_embeddings_js = np.abs(synth_standard_embeddings)
            synth_standard_embeddings_js /= np.sum(
                synth_standard_embeddings_js, axis=1, keepdims=True
            )

            train_val_distances = cdist(
                train_standard_embeddings_js, val_standard_embeddings_js, metric=method
            )
            adversarial_train_distances = cdist(
                train_adversarial_embeddings_js,
                train_standard_embeddings_js,
                metric=method,
            )
            adversarial_val_distances = cdist(
                val_adversarial_embeddings_js, val_standard_embeddings_js, metric=method
            )
            synth_train_distances = cdist(
                synth_standard_embeddings_js,
                train_standard_embeddings_js,
                metric=method,
            )
        else:
            train_val_distances = cdist(
                train_standard_embeddings, val_standard_embeddings, metric=method
            )
            adversarial_train_distances = cdist(
                train_adversarial_embeddings, train_standard_embeddings, metric=method
            )
            adversarial_val_distances = cdist(
                val_adversarial_embeddings, val_standard_embeddings, metric=method
            )
            synth_train_distances = cdist(
                synth_standard_embeddings, train_standard_embeddings, metric=method
            )

        if method in ["cosine", "correlation"]:
            min_train_val_distances = np.max(train_val_distances, axis=1)
            min_adversarial_train_distances = np.max(
                adversarial_train_distances, axis=1
            )
            ...
            min_adversarial_train_distances = np.min(
                adversarial_train_distances, axis=1
            )
        ...
        print(
            f"Distance shapes: train_val={min_train_val_distances.shape}, "
        ...
        # Compute detection ratios and Wasserstein distances
        if method in ["cosine", "correlation"]:
            ndpctl = np.percentile(results[method]["train_val"], 95)
        else:
            ndpctl = np.percentile(results[method]["train_val"], 5)

        adv_train_detection_count = np.sum(
            (results[method]["adversarial_train"] > ndpctl)
            if method in ["cosine", "correlation"]
            else (results[method]["adversarial_train"] < ndpctl)
        )
        adv_val_detection_count = np.sum(
            (results[method]["adversarial_val"] > ndpctl)
            if method in ["cosine", "correlation"]
            else (results[method]["adversarial_val"] < ndpctl)
        )

        total_adv_train = len(results[method]["adversarial_train"])
        total_adv_val = len(results[method]["adversarial_val"])

        dist_trval_synth = wasserstein_distance(
            results[method]["synth_train"], results[method]["train_val"]
        )
        dist_adv_train_trval = wasserstein_distance(
            results[method]["adversarial_train"], results[method]["train_val"]
        )
        dist_adv_val_trval = wasserstein_distance(
            results[method]["adversarial_val"], results[method]["train_val"]
        )
    ...
    json_file_path = os.path.join(
        output_dir,
        os.path.basename(embeddings_file).replace(
            ".h5",
            f"aug_{config['augmentation_strength']}_{config['loss_type']}_results.json",
        ),
    )
    ...
    plot_results(results, methods, output_dir, config, embeddings_file)
    ...
    for method, data in results.items():
        for key in ["train_val", "adversarial_train", "synth_train"]:
            ...
            if len(valid_data) > 0:
                ...
                print(
                    f"{method}_{key}: mean={stats[f'{method}_{key}_mean']}, median={stats[f'{method}_{key}_median']}"
                )
            else:
...
def plot_results(results, methods, output_dir, config, embeddings_file):
    n_methods = len(methods)
    n_cols = 2  # You can adjust this to change the number of columns
    n_rows = math.ceil(n_methods / n_cols)

    fig, axs = plt.subplots(n_rows, n_cols, figsize=(15 * n_cols, 8 * n_rows), dpi=300)
    ...
    for idx, (method, data) in enumerate(results.items()):
        ...
        for key in ["train_val", "adversarial_train", "adversarial_val", "synth_train"]:
            ...
            if len(valid_data) > 0:
                ...
                print(f"Warning: No valid data for {key} in {method}")
        ax.set_xlabel(
            f'{method.capitalize()} {"Distance" if method not in ["cosine", "correlation"] else "Similarity"}',
            fontsize=15,
        )
        ax.set_ylabel("Density", fontsize=15)
        ax.set_title(
            f'Distribution of {method.capitalize()} {"Distances" if method not in ["cosine", "correlation"] else "Similarities"}',
            fontsize=20,
        )
    ...
    plot_file = os.path.join(
        ...
        os.path.basename(embeddings_file).replace(
            ".h5",
            f"_aug_{config['augmentation_strength']}_{'-'.join(methods)}_{config['loss_type']}_distribution.png",
        ),
    )
...
def decode_byte_strings(data):
    if isinstance(data, bytes):
        ...
        return np.array(
            [item.decode("utf-8") if isinstance(item, bytes) else item for item in data]
        )
    else:
        return data
...
def find_and_plot_similar_images(
    embeddings_file,
    train_dataloader,
    val_dataloader,
    synth_dataloader,
    ...
    if not 0 <= plot_percentage <= 1 or not 0 < validation_percentage <= 1:
        raise ValueError(
            "plot_percentage and validation_percentage must be between 0 and 1"
        )
    ...
    print(
        f"Original shapes: train={train_standard_embeddings.shape}, val={val_standard_embeddings.shape}, synth={synth_standard_embeddings.shape}"
    )
    ...
    print(
        f"Subsampled shapes: train={train_standard_embeddings.shape}, val={val_standard_embeddings.shape}, synth={synth_standard_embeddings.shape}"
    )

    print("Computing distances between synthetic and training/validation images")
    train_distances = cdist(
        synth_standard_embeddings, train_standard_embeddings, metric="braycurtis"
    )
    val_distances = cdist(
        synth_standard_embeddings, val_standard_embeddings, metric="braycurtis"
    )
    ...
    output_dir = os.path.join(
        output_dir,
        f"synthetic_similar_pairs_{plot_percentage:.2f}_validation_{validation_percentage:.2f}_{extract_model_name(embeddings_file)}",
    )
    os.makedirs(output_dir, exist_ok=True)

    # Create sets of image IDs we need to load
    train_ids_to_load = set(
        train_image_ids[
            most_similar_train_indices[
                indices_to_plot[is_train_more_similar[indices_to_plot]]
            ]
        ]
    )
    val_ids_to_load = set(
        val_image_ids[
            most_similar_val_indices[
                indices_to_plot[~is_train_more_similar[indices_to_plot]]
            ]
        ]
    )
    ...
    print(
        f"IDs to load: train={len(train_ids_to_load)}, val={len(val_ids_to_load)}, synth={len(synth_ids_to_load)}"
    )
    ...
    def load_images(dataloader, ids_to_load, images_dict, set_name):
        ...
        for batch in tqdm(
            dataloader, total=int(len(dataloader) * validation_percentage)
        ):
        ...
        if ids_to_load:
            print(
                f"Warning: Could not find the following {set_name} images: {ids_to_load}"
            )
    ...
    for i, synth_idx in enumerate(tqdm(indices_to_plot)):
        ...
        if synth_img is None or real_img is None:
            print(
                f"Warning: Missing image for synthetic ID {synth_id} or {set_name} ID {real_id}"
            )
            continue
        ...
        plt.suptitle(
            f"Bray-Curtis Dissimilarity: {distance:.4f}\nRank: {i+1}/{num_images_to_plot}"
        )
        plt.tight_layout()

        output_file = os.path.join(
            output_dir,
            f"pair_{i:04d}_synth_{synth_id}_{set_name.lower()}_{real_id}.png",
        )
...
def visualize_augmentations(dataloader, num_images=6, num_augmentations=5):
    ...
    fig, axes = plt.subplots(
        num_images, num_augmentations + 1, figsize=(20, 4 * num_images)
    )
...
def visualize_augmentations_contrastive(dataloader, num_images=20, num_augmentations=4):
    ...
    fig, axes = plt.subplots(
        num_images, num_augmentations + 1, figsize=(20, 5 * num_images)
    )
    ...
    for i in range(num_images):
        ...
        subject = tio.Subject(
            image=tio.ScalarImage(tensor=anchor_img_gray.unsqueeze(0).unsqueeze(0))
        )
...
def rotate_2d(x: torch.Tensor, angle: float) -> torch.Tensor:
    ...
    rotation_matrix = torch.tensor([[cos, -sin, 0], [sin, cos, 0]])
    grid = torch.nn.functional.affine_grid(
        rotation_matrix.unsqueeze(0), x.unsqueeze(0).size(), align_corners=False
    )
    return torch.nn.functional.grid_sample(
        x.unsqueeze(0), grid, align_corners=False
    ).squeeze(0)
