...
def extract_features(
    network_name, model, images, processor=None, target_size=None, preprocess_fn=None
):
...
def initialize_model(network_name):
    ...
    if network_name.lower() == "clip":
        ...
        model = InceptionV3(
            weights=rad_imagenet_dict[network_name], include_top=False, pooling="avg"
        )
        target_size = (299, 299)
        preprocess_fn = inception_preprocess
...
def extract_features_from_directory(list_of_img, network_name, batch_size=16):
    ...
    if image_batch:
        batch_features = extract_features(
            network_name, model, image_batch, processor, target_size, preprocess_fn
        )
        features.append(batch_features)
        filenames.extend(batch_filenames)
